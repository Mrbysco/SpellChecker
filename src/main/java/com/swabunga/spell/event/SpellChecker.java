...
public class SpellChecker {
  ...
  public SpellChecker(SpellDictionary dictionary, int threshold) {
    this(dictionary);
    Configuration.SPELL_THRESHOLD = threshold;
  }
...
//    String previousWord = null;
    while (tokenizer.hasMoreWords() && !terminated) {
      ...
      if (!isCorrect(word)) {
          if ((Configuration.SPELL_IGNOREMIXEDCASE && isMixedCaseWord(word, tokenizer.isNewSentence())) ||
          ...
          if (!isIgnored(word)) {
            ...
            if (autoReplaceWords.containsKey(word)) {
              ...
              //Fire the event.
              List suggestions = getSuggestions(word, Configuration.SPELL_THRESHOLD);
  ...
  private List makeSuggestionsCapitalized(List suggestions) {
   ...
   private boolean isSupposedToBeCapitalized(String word, WordTokenizer wordTokenizer) {
     boolean configCapitalize = !Configuration.SPELL_IGNORESENTENCECAPITALIZATION;
